...
export default function Gaming() {
  ...
  const socialLinks = [
    { title: 'Discord', subtitle: 'Join the Server', icon: '💬', link: 'https://discord.gg/yourdiscord', gradient: 'from-indigo-500 to-purple-500' },
    { title: 'Instagram', subtitle: '@hawkkeyed', icon: '📸', link: 'https://instagram.com/hawkkeyed', gradient: 'from-pink-500 to-rose-500' },
    { title: 'YouTube', subtitle: 'Watch Highlights', icon: '▶️', link: 'https://youtube.com/@yourusername', gradient: 'from-red-500 to-orange-500' }
  ];
    ...
    <div className={`min-h-screen ${isDark ? 'bg-black text-white' : 'bg-white text-gray-900'} transition-colors duration-300`}>
      ...
      <main className='max-w-5xl mx-auto px-6 py-12'>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className={`space-y-6 text-lg leading-relaxed mb-16 ${isDark ? 'text-white/85' : 'text-gray-700'}`}>
          ...
          <p>Today, I channel the same competitive fire into <span className='text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 font-bold'>GameGrid.gg</span>, an esports analytics platform I'm building from scratch — combining gaming, AI, and data visualization to give players and teams the insights I once wished I had.</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className='mb-16'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            {socialLinks.map((social, i) => (
              <motion.a key={i} href={social.link} target='_blank' rel='noopener noreferrer' whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} className={`relative group rounded-2xl p-[1px] bg-gradient-to-r ${social.gradient}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className='mb-16'>
          <div className={`rounded-2xl ${isDark ? 'bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/20' : 'bg-gradient-to-r from-purple-100 to-pink-100 border-purple-300'} border-2 p-8 text-center`}>
            <p className='text-xl md:text-2xl font-bold italic'>💬 "In gaming and in life,<span className='text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400'> precision beats speed</span> — and <span className='text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400'> analysis wins over aggression</span>."</p>
          </div>
        </motion.div>
      ...
      <footer className={`w-full border-t ${isDark ? 'border-white/10 bg-black/30' : 'border-gray-200 bg-gray-50'} backdrop-blur-md mt-20`}>
        <div className='max-w-6xl mx-auto px-6 py-8'>
          <div className='flex flex-col md:flex-row items-center justify-between gap-4'>
            <div className={`text-center md:text-left ${isDark ? 'text-white/60' : 'text-gray-600'}`}>© 2025 Abhishek Jaisal — Founder & AI Enhanced Full-Stack Developer</div>
